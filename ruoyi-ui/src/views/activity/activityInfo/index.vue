  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="参与班级" prop="className">
          <el-select v-model="form.className" placeholder="请选择参与班级">
            <el-option v-for="item in classList" :key="item.className" :label="item.className" :value="item.className"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="活动地点" prop="activityPlace">
        ...
        <el-form-item label="活动类型" prop="activityType">
          <el-select v-model="form.activityType" placeholder="请选择活动类型">
            <el-option v-for="dict in dict.type.sys_activity_type" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="预设时长(分钟)" prop="presetDuration">
...
<script>
import { listActivity, getActivity, delActivity, addActivity, updateActivity, startActivity } from "@/api/system/activity"
...
export default {
  ...
  methods: {
    ...
    handleStart(row) {
      const ids = row.id || this.ids
       this.$modal.confirm('是否确认开始活动信息编号为"' + ids + '"的数据项？').then(function() {
        return getActivity(ids)
      }).then(response => {
