...
public class SysPurchaseController extends BaseController
{
    ...
    {
        startPage();
        List<SysPurchase> list = sysPurchaseService.selectPurchaseList(sysPurchase);
    ...
    {
        return success(sysPurchaseService.selectPurchaseById(id));
    }
    ...
    {
        return toAjax(sysPurchaseService.insertPurchase(sysPurchase));
    }
    ...
    {
        return toAjax(sysPurchaseService.updatePurchase(sysPurchase));
    }
    ...
    {
        return toAjax(sysPurchaseService.deletePurchaseByIds(ids));
    }

    @PreAuthorize("@ss.hasPermi('system:purchase:audit')")
    @Log(title = "审核采购单", businessType = BusinessType.UPDATE)
    @PutMapping("/audit")
    public AjaxResult audit(@RequestBody PurchaseAuditDTO dto) {
        return toAjax(sysPurchaseService.auditPurchase(dto));
    }
}
